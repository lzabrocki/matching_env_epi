...
We implement below a propensity score matching procedure where (i) we first predict the probability of the occurrence of an heat wave for each day using a simple logistic regression model and (ii) each day with an heat wave is then matched to the most similar day without heat wave. In the matching jargon, this is a pair nearest neighbor matching procedure without remplacement.
...
Once treated and control units are matched, we assess whether covariates has improved. We finally estimate the treatment effect.
