...
Rcpp::cppFunction('NumericMatrix pairDistCpp(NumericVector treated, NumericVector control) {
...
Rcpp::cppFunction('NumericMatrix pairAbsDistCpp(NumericVector treated, NumericVector control) {
            NumericMatrix D(treated.size(), control.size());
            for (int t = 0; t < treated.size(); t++) {
            for (int c = 0; c < control.size(); c++) {
            D(t,c) = std::abs(treated[t] - control[c]);
            }
            }
            return D;
}')
