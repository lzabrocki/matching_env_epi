...
The outputtells us that only `r matching_coarsened[["nn"]][3]` treated units were matched. We then evaluate the covariates balance using the `love.plot()` function from the cobalt package and the standardized mean difference as the summary statistic:
...
We find that the estimate for the average effect on the treated is equal to +`r results_cm_wo_cov$estimate` years of life lost. This is the true effect size. The 95% confidence interval is consistent with effects ranging from +`r results_cm_wo_cov$conf.low` up to +`r results_cm_wo_cov$conf.high`. If we want to increase the precision of our estimate and remove any remaining imbalance in covariates, we can also run a multivariate regression. We adjust below for the same variables used in the propensity score matching procedure:
...
bind_rows(
  ...
  mutate(
    ...
    sample_size = rep(matching_coarsened[["nn"]][3]*2, 2)
  ) %>%
  saveRDS(
    .,
    here::here(
