...
source(here::here("2.scripts", "functions", "script_time_series_matching_function.R"))
```
...
```{r, echo = TRUE, fig.width=8, fig.height=5, layout="l-body-outset", dev = "CairoPNG"}
# make stripes graph
matching_data %>%
...
```{r, echo=TRUE}
saveRDS(matching_data, here::here("3.outputs", "1.data", "constrained_pair_matching", "matching_data.rds"))
```
...
saveRDS(final_data, here::here("3.outputs", "1.data", "constrained_pair_matching", "matched_data.Rds"))
````

# Checking Covariates Balance Improvement

We bind the matching and matched data together:
...
# make the graph
ggplot(data_love_continuous, aes(y = time, x = standardized_difference, colour = fct_rev(dataset), shape = fct_rev(dataset))) +
...
```{R, echo=TRUE, message = FALSE, warning = FALSE, layout="l-body-outset", fig.width=10, fig.height=5, dev = "CairoPNG"}
# graph for weekdays
data_calendar %>%
...
```{R, echo=TRUE, message = FALSE, warning = FALSE, layout="l-body-outset", fig.width=8, fig.height=4, dev = "CairoPNG"}
# graph for month
data_calendar %>%
...
Must be careful, only `r N_matched` treated units were matched. To know the true causal effect, we must compute the average difference in potential outcomes:
...
We find that the average effect on the treated is equal to +`r results$estimate` years of life lost: the estimate is still very close to the true effect. The 95% confidence interval is consistent with effects ranging from +`r results$conf.low` up to +`r results$conf.high`.